...
var DebaterVideo = React.createClass({
  componentDidMount:function(){
   var session = OT.initSession(apiKey, sessionId)
        ...
        //   var subscriber = session.subscribe(event.stream, 'videoContainerCon', options);
        // })
        .connect(token, function(error) {    
          var options = {width: '100%', height: '100%', insertMode: 'append'}    
          var publisher = OT.initPublisher('videoContainerPro',options);
          session.publish(publisher);
        });
},
  render:function() {
    return (
          <div style={{width: this.props.videoWidth + "px", height:this.props.videoWidth + "px"}} className = "videoContainer" id ="videoContainerPro" >
          </div>
